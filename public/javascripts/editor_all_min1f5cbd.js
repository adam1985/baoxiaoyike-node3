...
//坑:UE会自己加载文件，不遵循原有缓存策略，所以一旦有改动需要更新版本号
UE.version = "1.2.6.1";
...
var utils = UE.utils = {
    ...
    loadFile:function () {
        ...
        return function (doc, obj, fn) {
            if( obj.src ) obj.src += '?v='+UE.version;
            if( obj.href ) obj.href += '?v='+UE.version;
...
var domUtils = dom.domUtils = {
    ...
    hasClass:function (element, className) {
        // fix element.className reports undefined of null bugs
        // 2014/05/13 by kainliu . bug by hugo
        if(!element){
            return false;
        }
...
//                });
        ...
        setContent: function (html, isAppendTo, notFireSelectionchange) {
            ...
            var root = UE.htmlparser(html,true);//@yinshen 改成不忽略空格，保持排版
...
//@raphealguo 
//这里要修改一下 新的图文消息正文图片上传的时候 要往img标签注入一个特殊的标志，标记它有640的缩略图。在手机加载图文消息的时候采用640的缩略图
UE.commands['insertimage'] = {
    execCommand:function (cmd, opt) {
        var sizes = "300,640";//目前支持300跟640 @date 2014/06/13 @raphealguo
...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            //这里要设置多一个属性标记640的图片
            if (first.src && /\/mmbizgif$/.test(first.src)){
                first.src = first.src.replace(/\/mmbizgif$/, "");
                if (first.data_ue_src && /\/mmbizgif$/.test(first.data_ue_src)){
                    first.data_ue_src = first.data_ue_src.replace(/\/mmbizgif$/, "");
                }
                domUtils.removeAttributes(img, "data-s");//如果是gif 不打标记 防止wap端显示缩略小图
            }else{
                first["data-s"] = sizes;
            }
            ...
            ci = opt[0];
            if (opt.length == 1) {
                //这里要打入标记"<img data-s=" + sizes
                var datas = ' data-s="' + sizes + '" ';
                if (ci.src && /\/mmbizgif$/.test(ci.src)){
                    ci.src = ci.src.replace(/\/mmbizgif$/, "");
                    datas = " ";
                }
                str = '<img ' + datas + ' src="' + ci.src + '" ' + (ci._src ? ' _src="' + ci._src + '" ' : '') +
                ...
                for (var i = 0; ci = opt[i++];) {
                    ...
                    //这里要打入标记"<img data-s=" + sizes
                    str = '<p ' + (ci['floatStyle'] == 'center' ? 'style="text-align: center" ' : '') + '><img ' + datas + ' src="' + ci.src + '" ' +
...
// @author:  kainliu
// @time： 2014/04/10
// @desc： add plugin for vote button
UE.plugins['insertvote'] = function (){
    var me = this;
    me.commands['insertvote'] = {
        execCommand: function () {
            window.openVotePopup();
        }
    }
    // leave interface for insert back
    window.insertVoteIframe = function(html){
...
// @author:  cunjinli
// @time： 2014/06/09
// @desc： add plugin for card button
UE.plugins['insertcard'] = function (){
    var me = this;
    me.commands['insertcard'] = {
        execCommand: function () {
            if (window.openCardSelect)
                window.openCardSelect();
        }
    }
    // leave interface for insert back
    window.insertCard = function(html){
        me.execCommand("inserthtml",html,true);
...
UE.plugins['video'] = function (){
    ...
    function creatInsertStr(url,width,height,align,toEmbed,addParagraph,className){
        return  !toEmbed ?
                ...
                (addParagraph?'</p>':'')
                :
                '<iframe ' + (className ? "class='" + className + "'" : "") +' style="position:relative; z-index:1;" height=' + height + ' width=' + width + ' frameborder=0 src="' + url + '" allowfullscreen></iframe><br/>';
                /*
                '<embed type="application/x-shockwave-flash" class="edui-faked-video" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
    ...
    me.commands["insertvideo"] = {
        execCommand: function (cmd, videoObjs){
            ...
            for(var i=0,vi,len = videoObjs.length;i<len;i++){
                ...
                html.push(creatInsertStr( vi.url, vi.width || 420,  vi.height || 280, vi.align||"none",true,true, "video_iframe"));
                // end mp.weixin.qq.com
            }
            me.execCommand("inserthtml",html.join(""),true);
            //
			var vid_container = document.createElement('div');
			vid_container.className = "js_vid";
			vid_container.setAttribute( "name", vi.vid );
			document.getElementsByTagName('body')[0].appendChild(vid_container);
            // for mp.weixin.qq.com
            /*var rng = this.selection.getRange();
...
(function () {
    ...
    var iframeUrlMap = {
        ...
        'insertvideo':'~/dialogs/video/video_v2.html',
    ...
    var btnCmds = ['undo', 'redo', 'formatmatch',
        ...
        'insertparagraphbeforetable', 'insertvote', 'insertcard', 'insertrow', 'insertcol', 'mergeright', 'mergedown', 'deleterow',
